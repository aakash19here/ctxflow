...
type UploadingFileUIPart = FileUIPart & { id: string; uploading?: boolean };

type AttachmentsContext = {
  files: UploadingFileUIPart[];
...
export const PromptInput = ({
  ...
  const add = useCallback(
    (files: File[] | FileList) => {
      ...
      const createItemFromFile = (file: File): InternalItem => ({
        id: nanoid(),
      ...
      const newItems: InternalItem[] = capped.map(createItemFromFile);
      ...
      for (let idx = 0; idx < capped.length; idx++) {
        ...
        (async () => {
          ...
          try {
            ...
            const { url, contentType, pathname } = data as any;
            setItems((prev) =>
              prev.map((it) =>
                it.id === id
                  ? {
            ...
            onError?.({
              code: "max_files",
              message: (err as any)?.message || "Failed to upload file",
            });
            // Remove failed item
            setItems((prev) => {
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    // Block submit if any uploads are in progress
    if (items.some((it) => it.uploading)) {
      return;
    }

    const files: FileUIPart[] = items
      .filter((it) => Boolean(it.serverUrl))
      .map((it) => ({
        type: "file",
        url: it.serverUrl as string,
        mediaType: it.mediaType || "application/octet-stream",
        filename: it.filename,
      }));

    onSubmit(
      { text: (event.currentTarget as any).message?.value, files },
      event
    );
    clear();
  };
  ...
  const ctx = useMemo<AttachmentsContext>(
    () => ({
      files: items.map((item) => ({
        id: item.id,
        type: "file",
        url: item.serverUrl ?? item.previewUrl,
